...
export const HistoryCard: React.FC<IHistoryCardProps> = ({
    ...
    useEffect(() => {
        axios.get("http://localhost:3001/api/todos").then((response: any) => {
            setHistoryTodos(response.data.filter((todo: ITodo) => todo.visible === false));
        });
    }, []);
    ...
    return (
        <div className="history-card">
            <h2 className="history-title">{historyTodos.length ? "Completed tasks" : "History list is empty"}</h2>
            <ul className={historyTodos.length > 8 ? "history-list scroll" : "history-list"}>
                {historyTodos.length
